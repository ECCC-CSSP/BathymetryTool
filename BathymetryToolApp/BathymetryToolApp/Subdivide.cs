...
{
    public void Subdivide(BathymetryToolAppForm form)
    {
        DirectoryInfo di = new DirectoryInfo(form.textBoxStartDir.Text);

        if (!di.Exists)
        {
            form.richTextBoxStatus.Text = "Directory does not exist [" + di.FullName + "";
            return;
        }
            ...
            if (fi.Name.StartsWith("N")) // ONNAP10_4600N06600W"))
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    for (int j = 0; j < 10; j++)
                    {
                        ...
                        //richTextBoxStatus.AppendText(newFileName + "\r\n");

                        lblStatus.Text = "doing --- " + di.FullName + "\\xyz\\" + newFileName + "nnnn.xyz";
                        lblStatus.Refresh();
                        ...
                        if (Line == null)
                        {
                            richTextBoxStatus.AppendText("Error: could not read first line\r\n");
                            return;
                        }
                        ...
                        if (countVal > 0)
                        {
                            ...
                            richTextBoxStatus.AppendText(fiNew.FullName + "\r\n");
